...
"""
Model Validation Script (Temporal CHARLS Cohort)
...
3.  Performs and plots Decision Curve Analysis (DCA) for 3-year risk.
"""
...
def run_external_validation_metrics():
    ...
    # Save results
    out_path = OUTPUT_DIR / "charls_validation_results.xlsx"
...
def run_risk_stratification_plots():
    ...
    if len(low_medium_scores) < 100:
        print(f"Low/Medium group too small, falling back to 2 groups.")
    ...
    for g in valid_groups:
        ...
        try:
            ...
            print(f"Error calculating incidence for group {g}: {e}")
            traceback.print_exc()
     
    if not incidence_data:
        print(" No incidence data calculated. Skipping bar plot.")
        return
    ...
    y_upper_limit = np.ceil(y_upper_limit * 10) / 10 
    y_upper_limit = min(y_upper_limit, 0.6) 
    ...
    filename_bar = (
        OUTPUT_DIR
        / f"Fig4A_{DATASET_NAME}_Incidence_BarPlot_{int(end_time)}yr_{group_suffix}.png"
    )
...
# ==================================
TIME_COL, EVENT_COL = "surv_time", "event"
TIME_HORIZON = 3.0  # 3-year temporal validation
...
# ==============================================================================
print("\n--- Step 1: Training CoxPH model on development set ---")
...
print("\n--- Step 2: Predicting 3-year risk on validation set ---")
df_val = pd.read_csv(VALID_PATH)
X_val = df_val[all_features]
surv_funcs = final_pipeline.predict_survival_function(X_val)

# Risk = 1 - S(3)
risk_probs = 1 - np.array([fn(TIME_HORIZON) for fn in surv_funcs])


print("\n--- Step 3: Landmark filtering for 3-year DCA ---")
df_dca = df_val.copy()
...
print(f"Original samples: {len(df_dca)}, After Landmark: {len(df_dca_filtered)}")
print(f"3-year event prevalence: {true_prevalence:.4f}")

# %%
# ==============================================================================
# 4. DCA Calculation (Code A Thresholds)
# ==============================================================================
print("\n--- Step 4: Running dcurves.dca ---")
thresholds = np.arange(0.005, 0.125, 0.005) 
...
# Smooth the curves using LOWESS
nb_m_s   = lowess(nb_m, pt, frac=0.25, it=0)[:, 1]
nb_all_s = lowess(nb_all, pt, frac=0.25, it=0)[:, 1]
...
plt.plot(pt, nb_none,  lw=1.5, color="gray", linestyle=":", label="Treat None")
...
plt.title("Temporal Validation (CHARLS 3-Year)", fontsize=22)

# Prevalence Vertical Line
plt.axvline(true_prevalence, ls=":", lw=1.5, color="gray")
plt.text(true_prevalence + 0.002, 0.005,
         f"Prevalence ≈ {true_prevalence:.3f}\n(Treat-all NB=0)",
         ha="left", fontsize=12, color="gray")

# Zero-Net Benefit Crossing
def calc_zero_nb(pt_arr, nb_arr):
    change_idx = np.where(np.sign(nb_arr[:-1]) != np.sign(nb_arr[1:]))[0]
    if len(change_idx) == 0: return None
    i = change_idx[-1]
    x0, x1, y0, y1 = pt_arr[i], pt_arr[i+1], nb_arr[i], nb_arr[i+1]
    return x0 - y0 * (x1 - x0) / (y1 - y0)

zero_nb = calc_zero_nb(pt, nb_m_s)
if zero_nb:
    plt.scatter([zero_nb], [0], s=40, color="#0072B2")
    plt.text(zero_nb + 0.002, 0.0015, f"Zero-NB ≈ {zero_nb:.2f}",
             ha="left", va="bottom", fontsize=9, color="#0072B2")

plt.legend(loc="upper right", fontsize=11, frameon=True, edgecolor="black", facecolor="white")
...
# Save
out_path = OUTPUT_DIR / "FigS3A_CHARLS_DCA_ValidationSet_3yr.png"
plt.savefig(out_path, dpi=300, bbox_inches="tight")
plt.show()

print(f"\n✅ DCA Figure saved to: {out_path}")
